...
exports.getAllByChatRoom = async (req, res) => {
  try {
    ...
    if (!user.chatRoomList.includes(req.params.id)) {
      return res
        .status(401)
        .json({ err: 'Unauthorised to view messages in this chat room' });
    }
    ...
    return res
      .status(500)
      .json({ err: 'An error occurred while searching for messages' });
  }
};
...
exports.getOneById = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ err: 'An error occurred while searching for message' });
  }
};
...
exports.createOne = async (req, res) => {
  try {
    ...
    if (!user.chatRoomList.includes(req.params.id)) {
      return res
        .status(401)
        .json({ err: 'Unauthorised to add messages to this chat room' });
    }
    ...
    // check the length of the trimmed message
    if (body.length === 0) {
    ...
    res.locals.io
      ...
      .emit('userStoppedTyping', user.username);
    ...
    return res
      .status(500)
      .json({ err: 'An error occured while creating message' });
  }
};
...
exports.changeOneById = async (req, res) => {
  try {
    ...
    if (body === message.body) {
      return res
        .status(422)
        .json({ err: 'No changes from original message body were provided' });
    }
    ...
    res.locals.io
      .to(patchedMessage.chatRoomId.toString())
      .emit('editMessage', patchedMessage);
    ...
    return res
      .status(500)
      .json({ err: 'An error occured while updating message' });
  }
};
...
exports.deleteOneById = async (req, res) => {
  try {
    ...
    res.locals.io
      .to(deletedMessage.chatRoomId.toString())
      .emit('deleteMessage', deletedMessage._id.toString());

    return res.json({ success: true });
  } catch (err) {
    return res
      .status(500)
      .json({ err: 'An error occured while deleting message' });
  }
};
