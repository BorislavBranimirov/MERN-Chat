...
exports.changeOneByName = async (req, res) => {
  ...
  try {
    ...
    if (isMatch) {
      return res
        .status(422)
        .json({ err: 'No changes from original user password were provided' });
    }
...
exports.deleteOneByName = async (req, res) => {
  ...
  try {
    ...
    for (const { _id } of chatRooms) {
      await Message.deleteMany({ chatRoomId: _id });
    }
    ...
    await User.updateMany(
      {},
      { $pull: { chatRoomList: { $in: chatRoomIds } } }
    );

    // delete all chat rooms owned by the user
    await ChatRoom.deleteMany({ admin: req.params.username });
