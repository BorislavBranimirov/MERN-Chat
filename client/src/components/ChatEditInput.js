...
const ChatEditInput = (props) => {
  ...
  const handleEditScroll = props.handleEditScroll;
  ...
  useLayoutEffect(() => {
    if (editInputFieldRef.current) {
      // resize text area to fit content
      const totalBorderHeight =
        editInputFieldRef.current.offsetHeight -
        editInputFieldRef.current.clientHeight;
      editInputFieldRef.current.style.height = '0';
      editInputFieldRef.current.style.height =
        editInputFieldRef.current.scrollHeight + totalBorderHeight + 'px';
      ...
      // this only matters if max-height was set to the edit field and the
      // edit message exceeds it, otherwise there is nothing to scroll to
      ...
      const bottomOfEdit =
        editInputFieldRef.current.closest('li').offsetTop +
        editInputFieldRef.current.closest('li').offsetHeight;
      handleEditScroll(bottomOfEdit);
    }
  }, [editInput, handleEditScroll]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setNotifications((notifications) => {
        return [
          ...notifications,
          { type: 'error', body: 'Error occurred while saving edited message' },
        ];
      });
    }
  };
  ...
  return (
    <div>
      ...
      <div className="chat-edit-input-btn-group">
        ...
        <input type="button" value="cancel" onClick={props.hideEditField} />
      </div>
    </div>
  );
};
