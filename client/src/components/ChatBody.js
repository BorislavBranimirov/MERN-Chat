...
const ChatBody = React.forwardRef((props, bodyRef) => {
  ...
  const messagesLoaded = useRef(null);
  const { loggedUser } = useContext(UserContext);
  const setNotifications = useContext(NotificationContext);
  const history = useHistory();

  useLayoutEffect(() => {
    // scroll to bottom of body when messages are first loaded
    if (!messagesLoaded.current && props.messages.length > 0) {
      messagesLoaded.current = true;
      scrollToBottom(bodyRef.current);
    }
  }, [bodyRef, props.messages.length]);
  ...
  const handleEditScroll = (bottomOfEdit) => {
    ...
    // offsetTop - offset from document body (space above chat body)
    // scrollTop - position of current scroll (scroll of chat body)
    // offsetHeight - height of chat body, including padding and borders
    ...
    // brings the entire message <li> into view, including buttons and padding
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setNotifications((notifications) => {
        return [
          ...notifications,
          { type: 'error', body: 'Error occurred while deleting message' },
        ];
      });
    }
  };
      ...
      <li key={props.messages[i]._id}>
        <div className="message-header">
          <div className="message-header-left">
            ...
            <small>
              {dayjs(props.messages[i].createdAt).format(
  ...
  return (
    <div ref={bodyRef} className="chat-body" onScroll={handleScroll}>
      {openModal && (
        <ConfirmationModal
