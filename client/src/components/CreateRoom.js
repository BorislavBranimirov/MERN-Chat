...
const CreateRoom = (props) => {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            delayedNotification.current = { type: 'standard', body: 'Room has been successfully created' };
            props.history.push('/chatrooms/' + resJSON.id);
        } catch (err) {
            setNotifications((notifications) => {
                return [...notifications, { type: 'error', body: 'Error occurred while creating chat room' }];
            });
        }
    };
